...
Framework7.prototype.plugins.k12 = function (app, params) {
	...
	var directory = 'github-personal/k12/k12-web/';
    ...
    // System functions - general usage
		...
		var get_ajax = function(url,data){
	        return $$.ajax({
		        type: "POST",
		        url: url,
		        data: {data: data},
	...
	// Getting the IP Address
		...
		var access_getIP = function(ip){
			var ajax = do_ajax('http://'+ip+'/k12/test.file','');
	...
	// main grading process
		...
		var _gradingSheet = function(){
			...
			$$('a[data-cmd="submit-form"]').on('click',function(){
				...
				get_grade(fields);
				get_student(fields);
		  		get_subjectDetails(fields);
				var content = do_ajax('templates/admin/grading-sheet.html','');
				innerContent(content.responseText);
				list_gradingSheet();
			});
		};
		var get_grade = function(controls){
			var nice_ip = localStorage.getItem('nice-ip');
			var data = do_ajax('http://'+nice_ip+'/'+processor+'?get-grade',controls);
			localStorage.setItem("grades_gradeSheetQuarter",data.responseText);
		};
		var get_student = function(controls){
			var nice_ip = localStorage.getItem('nice-ip');
		...
		var get_subjectDetails = function(controls){
			var nice_ip = localStorage.getItem('nice-ip');
		...
		var get_subjectsByYear = function(controls){
			var nice_ip = localStorage.getItem('nice-ip');
		...
		var get_gradeSummary = function(controls){
			var nice_ip = localStorage.getItem('nice-ip');
			var data = do_ajax('http://'+nice_ip+'/'+processor+'?get-gradeSummary',controls);
		...
		var list_summaryGrade = function(){
			...
			$$('a[data-cmd="submit-form"]').on('click',function(){
				...
				localStorage.setItem('controls_gradingSheet',fields);
				get_student(fields);
		  		get_subjectsByYear(fields);
		  		get_gradeSummary(fields);
				// var content = do_ajax('templates/admin/grading-sheet.html','');
				// innerContent(content.responseText);
				// list_gradingSheet();
			});
