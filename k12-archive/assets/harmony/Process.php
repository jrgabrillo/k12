...
$function = new DatabaseClasses;
	...
	if(isset($_GET['createDB'])){
		$data = $function->createDB('temp_k12');
		if($data == 1){
			echo 1;
		}
	}
	...
	if(isset($_GET['login'])){
		...
		$query = $function->PDO(true,"USE temp_k12; SELECT * FROM tbl_user WHERE username = '{$username}' AND password = '{$password}'");
    ...
    if (isset($_GET['get-studentsPromotion'])) {
    	$final = [];
    	$data = $_POST['data'];    	
    	$data = json_decode($data);
    	$year = $data[0]->value;
        ...
        foreach ($data as $key => $value) {
			...
			if(count($query)<=0){
		        ...
		        if((count($data)-1) <= $key){
					...
					$q2 .= "('{$id}','{$value[0]}','{$value[6]}','{$value[7]}','{$date}','0')";
		        }
		        else{
					$q1 .= "('{$id}','{$value[1]}','{$value[2]}','{$value[3]}','{$value[5]}','{$value[4]}','{$value[13]}','{$value[14]}','{$value[9]}','{$value[10]}','{$value[11]}','{$value[12]}','{$value[8]}','{$value[0]}','avatar.jpg','{$date}'),";
    ...
    if (isset($_GET['set-studentsPromotion'])) {
    	$data = $_POST['data'];
    	$data = json_decode($data);
    	$year = $data[(count($data)-3)]->value;
    	$section = $data[(count($data)-2)]->value;
    	$q = "";

    	foreach ($data as $i => $v) {
    		if($v->value == "on"){
    			$q .= "UPDATE tbl_student SET year = '".$year."', section = '".$section."' WHERE student_id = '".$v->name."';";
    		}
    	}

		$query = $function->PDO(false,$q);
		if($query->execute()){
			echo 1;
		}
		else{
    ...
    if(isset($_GET['drop-tempdatabase'])){
		$data = $function->dropDB('temp_k12');
		echo 1;
    }
